...
function run() {
  ...
  assert.ok(html.includes('data-step="0.1"'), "step presets should include 0.1");
  ...
  assert.ok(html.includes('id="btnCopy"'), "copy button should expose a stable id");
  assert.ok(html.includes('id="toggleAllResults"'), "bulk select toggle should be present");
  assert.ok(html.includes('id="selectedCount"'), "selected count should be present");
  assert.ok(html.includes('class="result-list"'), "result list container should be present");
  assert.ok(!html.includes("<table>"), "legacy result table should be removed");
  assert.ok(html.includes('class="query-note query-note-inline"'), "query note should use the compact inline layout");
  ...
  assert.ok(appJs.includes("renderLoadingState"), "app should render loading state for remote sync");
  assert.ok(appJs.includes("renderErrorState"), "app should render error state when remote data fails");
  assert.ok(appJs.includes("syncToggleAllState"), "app should keep bulk select state in sync");
  assert.ok(appJs.includes("updateSelectionUi"), "app should update selected count UI");
  assert.ok(appJs.includes("applyManualDiscount"), "app should support manual discount input");
  assert.ok(appJs.includes("discount-manual"), "app should render discount input fields");
  assert.ok(appJs.includes("matches.length === 1"), "single match rows should default to checked");
  assert.ok(appJs.includes('"result-card"'), "app should render result cards");
  ...
  assert.ok(css.includes(".discount-input-shell"), "styles should include manual discount input shell");
  assert.ok(css.includes(".discount-manual"), "styles should include manual discount input");
  ...
  assert.ok(css.includes(".result-card"), "styles should include card-based result layout");
  assert.ok(css.includes(".result-row"), "styles should include compact result rows");
  assert.ok(css.includes(".select-all-toggle"), "styles should include bulk select toggle");
  assert.ok(css.includes(".state-card"), "styles should include empty/loading/error states");
  assert.ok(html.includes('class="result-stat"'), "result stat badge should be present");
  assert.ok(!html.includes("模糊匹配"), "page should not show fuzzy-match labels");
